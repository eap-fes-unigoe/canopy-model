...
hybrid_root_ci = function(met,state_last,pars,ps_sc, xa, xb, tol){
  ...
  x0 = xa;
  flux_f0 = CiFunc(met,state_last,pars,ps_sc, x0);
  flux = flux_f0[[1]]
  f0 = flux_f0[[2]]
  if (f0 == 0) {
    root = x0;
  ...
  x1 = xb;
  flux_f1 = CiFunc(met,state_last,pars, x1);
  flux = flux_f1[[1]]
  f1 = flux_f1[[2]]
  if (f1 == 0) {
    root = x1;
  ...
  for (iter in 1:itmax){
    ...
    f0 = f1;
    x1 = x;
    flux_f1 = CiFunc (met,state_last,pars,ps_sc, x1);
    flux = flux_f1[[1]]
    f1 = flux_f1[[2]]
    if (f1 < minf){
      minx = x1;
    ...
    if (f1 * f0 < 0){
      flux_x = brent_root_ci (met,state_last,pars,ps_sc, x0, x1, tol);
      flux = flux_x[[1]]
      x = flux_x[[2]]
      x0 = x;
      break
    ...
    if (iter == itmax) {
      flux_f1 = CiFunc (met,state_last,pars,ps_sc, minx);
      flux = flux_f1[[1]]
      f1 = flux_f1[[2]]
      x0 = minx;
    }
