...
night_bg <- function(){
  list(
    geom_tile(aes(x= datetime, width = dt, y = (max(radiation) + min(radiation))/2, height = max(radiation) - min(radiation) +30, fill = night), alpha = .05, linetype = 0),
  scale_fill_manual(name = NULL, values = c("white", "black"), labels = c("day", "night"),
                    guide = guide_legend(override.aes = list(alpha = .2))),
  theme_bw())
}
...
days_out %>%
  ...
  geom_tile(aes(x = datetime, width = 3600, y = (max(radiation) + min(radiation))/2, height = max(radiation) - min(radiation) +10, fill = night), alpha = .05, linetype = 0) +
        labs(y="Outgoing shortwave radiation (w m-2)", str_wrap("Outgoing shortwave radiation", 10)) +
...
```{r}
ggplot(out_plot) +
  geom_abline(aes(intercept = 0, slope = 1, color = "theory"), key_glyph = "abline") +
